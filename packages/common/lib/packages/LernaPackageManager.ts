...
export class LernaPackageManager implements ILernaPackageManager {
  ...
  public async init(): Promise<void> {
    ...
    this.logger.debug(`Executing: ${DEFAULT_COMMANDS.PACKAGES}`);

    const packagesConfig: PackageConfig[] = JSON.parse(ProcessExecutor.execute(DEFAULT_COMMANDS.PACKAGES));

    this.logger.debug(`Found ${packagesConfig.length} lerna packages`);
...
export function lernaPackageManagerFactory(logger: ILogger): ILernaPackageManager {
